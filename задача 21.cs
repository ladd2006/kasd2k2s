...
using static задача_21.Program;
...
namespace задача_21
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"Анна: {map.get("Анна")}");
            Console.WriteLine($"Борис: {map.get("Борис")}");
            Console.WriteLine($"Есть Анна: {map.containsKey("Анна")}");
            Console.WriteLine($"Есть Петр: {map.containsKey("Петр")}");
            Console.WriteLine($"Есть значение 25: {map.containsValue(25)}");

            map.put("Анна", 26);
            Console.WriteLine($"Анна после обновления: {map.get("Анна")}");
            ...
            Console.WriteLine($"\nУдаляем Бориса: {map.remove("Борис")}");
            Console.WriteLine($"Размер после удаления: {map.Size()}");

            Console.WriteLine($"\nТаблица пуста: {map.isEmpty()}");

            map.clear();
            Console.WriteLine($"Размер после очистки: {map.Size()}");
            Console.WriteLine($"Таблица пуста: {map.isEmpty()}");

            MyHashMap<int, string> map2 = new MyHashMap<int, string>(4, 0.75f);
            for (int i = 0; i < 10; i++)
            {
                map2.put(i, $"Элемент{i}");
            }
            Console.WriteLine($"\nПосле 10 элементов: size={map2.size}, count={map2.count}");
        }
    }
}
